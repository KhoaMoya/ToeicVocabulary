...
interface CategoryDao {
    ...
    @Query("update category set known=(select count(*) from word where word.category_id = category.id and word.status = 1) where category.id =:id")
    fun updateKnownWords(id: Int)
